// components/orders/partials/OrderItemsTable.tsx
...
export function OrderItemsTable({ items, hideTopBorder = false }: OrderItemsTableProps) {
    ...
    return (
        <div className={`mt-3 ${hideTopBorder ? '' : 'border-t'} border-gray-200 dark:border-gray-700 ${hideTopBorder ? '' : 'pt-3'}`}>
            ...
            <div className="overflow-x-auto">
                <table className="min-w-full text-sm">
                    ...
                    {items.map((item) => {
                        ...
                        return (
                            <tr key={item.id}>
                                <td className="px-3 py-2 text-gray-500 dark:text-gray-400 font-mono">#{item.id}</td>
                                <td className="px-3 py-2 text-gray-500 dark:text-gray-400 font-mono">{item.inventory_id}</td>
                                <td className="px-3 py-2 text-gray-900 dark:text-gray-100">{menuItemName}</td>
                                <td className="px-3 py-2 text-gray-600 dark:text-gray-400">{categoryName}</td>
                                <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">{item.quantity}</td>
                                <td className="px-3 py-2 text-right text-gray-600 dark:text-gray-400">₱{unitPrice.toFixed(2)}</td>
                                <td className="px-3 py-2 text-right font-medium text-gray-900 dark:text-gray-100">₱{Number(item.amount).toFixed(2)}</td>
                            </tr>
                        );
                    })}
                    </tbody>
